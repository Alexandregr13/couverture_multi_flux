...
int main(int argc, char** argv) {
    ...
    if (!portfolioPath.empty()) {
        ...
        if (portfolioFile.is_open()) {
            ...
            if (portfolio.is_array() && portfolio.size() > 0) {
                auto& firstEntry = portfolio[0];
                expectedPrice = firstEntry["price"].get<double>();
                expectedDeltas = firstEntry["deltas"].get<vector<double>>();
                if (firstEntry.contains("priceStdDev")) {
                    expectedPriceStdDev = firstEntry["priceStdDev"].get<double>();
                }
                if (firstEntry.contains("deltasStdDev")) {
                    expectedDeltasStdDev = firstEntry["deltasStdDev"].get<vector<double>>();
                }
            }
        }
    }
